...
class display {
  static appendAllTodos(projectName, sortSelected) {
    ...
    for (let i = 0; i < myToDosSorted.length; i++) {
      if (myToDosSorted[i].project == projectName) {
        todoContainer.appendChild(document.createElement('li')).className =
          'listItem';
        todoContainer.lastChild.id = 'task' + i;
        todoContainer.lastChild.appendChild(
          document.createElement('label')
        ).className = 'checkLabel';
        todoContainer.lastChild
          .querySelector('.checkLabel')
          .setAttribute('for', `${i}`);
        todoContainer.lastChild.firstChild.appendChild(
          document.createElement('span')
        ).className = 'priority';
        todoContainer.lastChild.querySelector(
          '.priority'
        ).style.backgroundColor = myToDosSorted[i].priority;
        todoContainer.lastChild.firstChild.appendChild(
          document.createElement('input')
        ).id = i;
        document.getElementById(i).setAttribute('type', 'checkbox');
        if (
          myToDosSorted[i].checked == 'true' ||
          myToDosSorted[i].checked == true
        ) {
        ...
        todoContainer.lastChild.querySelector(
          '.labelDisc'
        ).textContent = discName;
        todoContainer.lastChild
          .querySelector('.labelDisc')
          .setAttribute('href', '#');
        todoContainer.lastChild.firstChild.appendChild(
          document.createElement('a')
        ).className = 'time';
        todoContainer.lastChild.querySelector('.time').textContent = format(
          parseISO(myToDosSorted[i].time),
          'MM/dd/yyyy'
        );
        todoContainer.lastChild
          .querySelector('.time')
        ...
        todoContainer.lastChild
          ...
          .appendChild(document.createElement('i')).className =
          'fa fa-pencil-square-o';
        todoContainer.lastChild.firstChild.appendChild(
          document.createElement('button')
        ).className = 'delete';
        todoContainer.lastChild.querySelector('.delete').id = i + 'delete';
        todoContainer.lastChild.firstChild
          .querySelector('.delete')
          .appendChild(document.createElement('i')).className = 'fa fa-trash';
      }
    }
  ...
  static get appendAllProjects() {
    ...
    for (let i = 0; i < myProjects.length; i++) {
      projectContainer.appendChild(document.createElement('label')).className =
        'projectListing';
      projectContainer.lastChild
        .appendChild(document.createElement('input'))
        .setAttribute('name', 'radio');
      projectContainer.lastChild.querySelector('input').type = 'radio';
      projectContainer.lastChild.appendChild(
        document.createElement('span')
      ).className = 'projSelect';
      projectContainer.lastChild.querySelector('.projSelect').textContent =
        myProjects[i];
      projectContainer.lastChild
        .querySelector('.projSelect')
        .appendChild(document.createElement('button')).className = 'projDelete';
      projectContainer.lastChild.querySelector('.projDelete').id = i + 'delete';
      projectContainer.lastChild
        .querySelector('.projDelete')
        .appendChild(document.createElement('i')).className = 'fa fa-trash';
    }
  }
  ...
  static infoScreen(key) {
    ...
    contentDiv.appendChild(document.createElement('div')).className =
      'formContainer';
    contentDiv.lastChild.appendChild(document.createElement('form')).className =
      'discForm';
    let discScreen = document.querySelector('.discForm');
    discScreen.appendChild(document.createElement('button')).className =
      'closebtn';
    discScreen.querySelector('.closebtn').setAttribute('type', 'reset');
    discScreen
      .querySelector('.closebtn')
      .appendChild(document.createElement('i')).className = 'fa fa-times';
    discScreen.appendChild(document.createElement('h1')).textContent =
      'ToDo Description';
    discScreen.appendChild(document.createElement('p')).textContent =
      'Date Due:' + ' ' + format(parseISO(myToDos[key].time), 'PPPP');
    let timeRemaining = '';
    if (
      formatDistanceToNow(endOfDay(parseISO(myToDos[key].time)), {
        addSuffix: true,
      }).includes('ago')
    ) {
      timeRemaining = 'Past due';
    } else {
      timeRemaining = formatDistanceToNow(
        endOfDay(parseISO(myToDos[key].time)),
        { addSuffix: true }
      );
    }
    discScreen.appendChild(document.createElement('p')).textContent =
      'Time remaining to complete task:' + ' ' + timeRemaining + '!';
    discScreen.appendChild(document.createElement('h3')).textContent =
      'Description expanded:';
    discScreen.appendChild(document.createElement('p')).textContent =
      myToDos[key].description;
