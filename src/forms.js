...
class forms {
  static todoForm(status, clickevent) {
    ...
    if (status == 'new') {
      ...
      form.appendChild(document.createElement('h1')).textContent =
        'Modify ToDo';
    }
    form.appendChild(document.createElement('p')).textContent =
      'Project select';
    form.appendChild(document.createElement('select')).id = 'projectChoice';
    const projectChoices = form.querySelector('#projectChoice');
    for (let i = 0; i < myProjects.length; i++) {
      projectChoices.appendChild(document.createElement('option')).textContent =
        myProjects[i];
    }
    ...
    const priorityChoices = form.querySelector('#priorityChoice');
    priorityChoices.appendChild(document.createElement('option')).textContent =
      'Low';
    ...
    if (status == 'modify') {
      form.querySelector('input').value = format(
        new Date(myToDos[key].time),
        'yyyy-MM-dd'
      );
    }
  ...
  static get newProjectForm() {
    projectContainer.appendChild(document.createElement('form')).id =
      'newProjForm';
    projectContainer
      .querySelector('#newProjForm')
      .setAttribute('submit', 'e.preventDefault();');
