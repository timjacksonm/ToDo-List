...
class screenEvent {
      ...
      button.addEventListener('click', (e) => {
        if (e.target.className == 'fa fa-trash') {
          ...
          const choice = confirm(
            'Delete' +
              ' ' +
              `"${myProjects[projIndex]}"?` +
              ' ' +
              'Confirm will delete this project and all todo/s assigned to it.'
          );
          if (choice == true) {
            let todoIndex = 0;
  ...
  static addFormListener(status, key) {
    ...
    formSelect.querySelector('.closebtn').addEventListener('click', () => {
      forms.closeToDoForm;
    });
    switch (status) {
      case 'new':
  ...
  static getToDoFormData(formstatus, key) {
    ...
    if (formstatus == 'new') {
      if (completed == true) {
        myToDos.push(
          new ToDo(
            `${formSelect.querySelector('#projectChoice').value}`,
            `${formSelect.querySelector('#priorityChoice').value}`,
            'no',
            `${formSelect.querySelector('textarea').value}`,
            `${formSelect.querySelector('input').value}`
          )
        );
        local.storeData;
        return true;
      ...
      if (completed == true) {
        myToDos[key].setProject(
          formSelect.querySelector('#projectChoice').value
        );
        myToDos[key].setPriority(
          formSelect.querySelector('#priorityChoice').value
        );
