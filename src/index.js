...
const todo9 = new ToDo(
  ...
  'Green',
  'false',
  'Create a new project in the nav menu! Be careful not to delete a project that has task you want to keep!',
...
const todo6 = new ToDo(
  ...
  'Orange',
  'false',
  'This task test that (checked) works as intended and is saved if you switch to a different project.',
  `${format(addDays(new Date(), 3), 'yyyy-MM-dd')}`
);
const todo5 = new ToDo(
  ...
  'Green',
  'false',
  'This task test the character limit for this description title. Once you click the description title a new info window will appear. You can read this full description inside that window!',
  `${format(new Date(), 'yyyy-MM-dd')}`
);
const todo4 = new ToDo(
  ...
  'Green',
  'false',
  'Go ahead Add a new Task to this project by clicking the + button above!',
  `${format(new Date(), 'yyyy-MM-dd')}`
);
const todo3 = new ToDo(
  ...
  'Red',
  'false',
  'This task is displayed for All, Month. Unless you are veiwing this page within the last 8 days of the current month!',
  `${format(addDays(new Date(), 8), 'yyyy-MM-dd')}`
);
const todo2 = new ToDo(
  ...
  'Orange',
  'false',
  'This task is displayed for this Week / Month. Unless you are veiwing this page on the last day of the current week!',
...
const todo1 = new ToDo(
  ...
  'Green',
  'false',
  'This task is displayed for any sort selection.',
  `${format(new Date(), 'yyyy-MM-dd')}`
);
...
(function winLoad() {
  ...
  projectContainer.addEventListener('submit', (e) => {
    e.preventDefault();
    if (
      myProjects.includes(e.target.firstChild.value) ||
      e.target.firstChild.value == ''
    ) {
      e.target.firstChild.required = true;
      document.querySelector(
        '#newProjForm > input[type=text]'
  ...
  window.addEventListener('click', (e) => {
    if (
      e.target.offsetWidth > document.getElementById('mySidenav').offsetWidth &&
      document.getElementById('mySidenav').offsetWidth > 100
    ) {
      display.closeNav;
    }
  });
  ...
  if (localStorage.length == 0) {
    ...
    todoDataFromLocal.forEach((todo) => {
      myToDos.push(
        new ToDo(
          `${todo.project}`,
          `${todo.priority}`,
